...
Forest Arena는 독창적인 3등신 동물 캐릭터가 싸우는 Godot 4 기반 Android 우선 2D 플랫폼 아레나 격투게임이다. 현재 저장소는 Phase 3 전투 원형 비교용 데이터·런타임 기반을 포함한다.
...
`./scripts/verify.sh`는 headless 편집기 로드, Phase 0 기반 smoke, Phase 1 전투, Phase 2 로드아웃, Phase 3 전투 원형, manifest 기반 CharacterData·캐릭터 외형·승인 모션 계약, 공격·비전투 UI 설계 계약, 하네스 구조 검사를 묶는 기본 로컬 검증이다. `./scripts/verify-harness.sh`는 스킬·문서·라우팅만 빠르게 확인한다.

## 현재 상태

- 완료: Phase 1 결정론적 전투 수직 슬라이스(PR #6, `665757b`), Godot 리소스 카탈로그(PR #7, `038090b`), 그리고 Phase 2 버전 로드아웃 계약·런타임 주입.
- 현재: Phase 3 원형 비교 구현과 자동 회귀는 완료됐으며, 14개 역할 하네스가 활성 역할의 단일 원본이다. Phase 종료는 Galaxy S23 Ultra 실기기 18매치·10분 성능 증적 및 독립 QA가 남아 있어 아직 선언하지 않는다.
