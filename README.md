...
- 로컬 AI 플레이 작업 브랜치: 첫 캐릭터 지급, 0원 상점·기기 저장, 네 캐릭터 선택, AI 한 판과 재대전이 연결됐다. [플레이 안내](docs/gameplay/local-ai-play.md)를 참고한다. 신규 전투 모션 승인·등록과 이번 버전 실기기 확인이 남아 있으며, 이 구현만으로 공식 Phase 6 전체를 완료 처리하지 않는다.
- 완료: 같은 입력이면 같은 결과가 나오는 기본 전투 한 판(Phase 1), 이미지·소리를 안정적인 이름으로 찾는 목록, 캐릭터·직업·장신구를 조합하는 데이터 처리(Phase 2).
- 현재: AI 작업은 14개 전문 역할로 나눈다. 정식 선택 화면, 성장, 게임 재화와 Phase 4 이후의 확장 기능은 아직 없다.
