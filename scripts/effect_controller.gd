...
static func dispatch(trigger: int, owner: FighterController, other: FighterController, rules: CombatRules, cause_id: StringName = &"", chain_depth := 0, tags: Array[StringName] = []) -> void:
	...
	for effect: Resource in owner.runtime_profile.combat_effects:
		...
		var required_tags: Array[StringName] = effect.get("tags") as Array[StringName]
		if not required_tags.is_empty() and not required_tags.any(func(tag: StringName) -> bool: return tags.has(tag)): continue
		match int(effect.get("kind")):
			...
			EffectData.Kind.GAIN_ULTIMATE:
				owner.runtime_state.ultimate_gauge = minf(rules.ultimate_gauge_max, owner.runtime_state.ultimate_gauge + float(effect.get("amount")))
			EffectData.Kind.REFLECT_DAMAGE, EffectData.Kind.EXPLOSION_DAMAGE:
				...
				dispatch(EffectData.Trigger.ON_DAMAGED, other, owner, rules, StringName(effect.get("cause_id")), chain_depth + 1, tags)
