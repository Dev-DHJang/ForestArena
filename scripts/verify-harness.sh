...
for path in AGENTS.md README.md project.godot export_presets.cfg docs/INDEX.md docs/GLOSSARY.md docs/product/vision.md docs/gameplay/combat.md docs/gameplay/features-and-ux.md docs/engineering/architecture.md docs/content/art-and-audio.md docs/product/roadmap.md docs/operations/ai-development.md docs/product/decisions.md docs/contracts/character-appearance-v01.json docs/ui/non-combat-ui-v01.json docs/ui/penpot-setup.md tests/ui_design_contract.gd assets/ui/.gdignore assets/ui/asset-requirements.csv .codex/config.toml.example scripts/start-penpot-mcp.sh scripts/verify-docs.sh docs/operations/harness/team-spec.md docs/operations/harness/operating-index.md
do
  check_file "$path"
done

team_skills=$(awk -F'|' '/^\|/ && $3 ~ /^[[:space:]]*forest-arena-/ {gsub(/^[[:space:]]+|[[:space:]]+$/, "", $3); print $3}' docs/operations/harness/team-spec.md)
...
if rg -n 'forest-arena-(godot-ui|character-motion|version-control)' AGENTS.md README.md docs/operations docs/engineering/godot docs/ui .agents; then fail "removed skill ID in active guidance"; fi
if rg -n '[1]7개 역할|[1]5개 역할|현재 Phase 0' AGENTS.md README.md docs/product/vision.md docs/gameplay/combat.md docs/gameplay/features-and-ux.md docs/operations/ai-development.md docs/operations docs/ui .agents; then fail "stale active role count or Phase state"; fi

grep -qF 'ForestArenaResources' .agents/skills/forest-arena-ui/SKILL.md || fail "UI resource consumption missing"
grep -qF 'docs/engineering/godot/setup.md' .agents/skills/forest-arena-ui/SKILL.md || fail "UI setup missing"
...
grep -qF '00_request.md' docs/operations/harness/team-spec.md || fail "request evidence missing"
grep -qF '04_closeout.md' docs/operations/harness/team-spec.md || fail "closeout evidence missing"
if grep -qF '02_version'"-control.md" docs/operations/harness/team-spec.md docs/operations/ai-development.md; then fail "obsolete evidence remains"; fi

for path in docs/engineering/godot/setup.md docs/engineering/godot/resource-rules.md forest_arena/scripts/forest_arena_resource_manager.gd forest_arena/data/resource_registry.json forest_arena/data/quality_profiles.json tools/forest_arena/verify_godot_resources.py harness/forest_arena_godot/codex_apply_contract.json
do
  check_file "$path"
done
...
do
  grep -qF 'docs/contracts/character-appearance-v01.json' ".agents/skills/forest-arena-$consumer/SKILL.md" || fail "appearance consumer: $consumer"
done
grep -qF '사용자 시안 승인만으로는 계약 변경 승인이 되지 않는다' docs/operations/harness/team-spec.md || fail "appearance contract gate missing"
...
grep -qF 'Penpot 비전투 UI 설계' docs/operations/harness/team-spec.md || fail "Penpot route missing"
...
if grep -REn "$legacy" .agents/skills docs/operations/harness docs/ui; then fail "legacy brand in active guidance"; fi
