...
여기서 논리 ID는 실제 파일 경로 대신 쓰는 안정적인 자산 이름이다. 자세한 설명은 [용어 가이드](../../GLOSSARY.md#godot와-데이터-용어)를 참고한다.
...
- 자산 연결 목록을 바꾼 뒤 `python tools/forest_arena/verify_godot_resources.py --project-root .`로 ID와 파일 연결, 품질별 파일과 시작 시 자동 등록을 검사한다.
